 ...
 * @version 3.17.23
 * 
 * Added grid lines to the grid and a message indicating loss.
 * 
 * @author Anu Datar
 * 
...
public class BlockDisplay extends JComponent implements KeyListener
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        for (int row = 0; row < board.getNumRows(); row++)
        {
            g.drawLine(0, row * BLOCKSIZE, board.getNumCols() * BLOCKSIZE, row * BLOCKSIZE);
            g.setColor(Color.darkGray);
        }
        for (int col = 0; col < board.getNumCols(); col++)
        {
            g.drawLine(col * BLOCKSIZE, 0, col * BLOCKSIZE, board.getNumRows() * BLOCKSIZE + OUTLINE);
            g.setColor(Color.darkGray);
        }
    }
    ...
    public void showLoss()
    {
        ...
        //frame.add(label1);
        //lost = true;
        JOptionPane.showMessageDialog(null, "You lost!","Tetris", JOptionPane.PLAIN_MESSAGE);
        /*
        Container c = frame.getContentPane();
        JLabel l = new JLabel("You lost!");
        l.setFont(new Font("Ariel", Font.BOLD, 28));
